...
func GetUsers(w http.ResponseWriter, r *http.Request) {
	...
	result, err := db.Query("SELECT id, date," +
		"description, amount, status, receiver, jk, no_telp, address from transaksi_keuangan_riza")
	if err != nil {
		panic(err.Error())
	}
	defer result.Close()
...
func CreateUser(w http.ResponseWriter, r *http.Request) {
	w.Header().Set("Content-Type", "application/json")
	stmt, err := db.Prepare("INSERT INTO transaksi_keuangan_riza(date," +
	...
	jk := keyVal["jk"]
	noTelp := keyVal["no_telp"]
	address := keyVal["address"]
	_, err = stmt.Exec(date, description, amount, status, receiver, jk, noTelp, address)
	if err != nil {
		panic(err.Error())
	}
...
func GetUser(w http.ResponseWriter, r *http.Request) {
	...
	params := mux.Vars(r)
	result, err := db.Query("SELECT id, date,"+
		"description, amount, status, receiver, jk, no_telp, address from transaksi_keuangan_riza WHERE id = ?", params["id"])
	if err != nil {
		panic(err.Error())
	}
	defer result.Close()
...
func UpdateUser(w http.ResponseWriter, r *http.Request) {
	w.Header().Set("Content-Type", "application/json")
	params := mux.Vars(r)
	stmt, err := db.Prepare("UPDATE transaksi_keuangan_riza SET date = ?," +
	...
	noTelp := keyVal["no_telp"]
	address := keyVal["address"]
	_, err = stmt.Exec(date, description, amount, status, receiver, jk, noTelp, address, params["id"])
	if err != nil {
		panic(err.Error())
	}
...
func DeleteUser(w http.ResponseWriter, r *http.Request) {
	w.Header().Set("Content-Type", "application/json")
	params := mux.Vars(r)
	stmt, err := db.Prepare("DELETE FROM transaksi_keuangan_riza WHERE id = ?")
	if err != nil {
		panic(err.Error())
	}
...
func InitDB() {
	db, err = sql.Open("mysql", "root:@tcp(127.0.0.1:3306)/db_2200908_muhammadrizaabqari_uas_pilkomb")
	if err != nil {
		panic(err.Error())
	}
}
